...
export default function ProfilePage() {
  const [formData, setFormData] = useState({ 
      name: '', designation: '', bio: '', profileImage: '', cvUrl: '',
      socialLinks: { github: '', linkedin: '', twitter: '' },
      about: { paragraphs: [''] }, // Initialize with one empty paragraph
  ...
  const removeParagraph = (index) => {
    if (formData.about.paragraphs.length <= 1) return; // Always keep at least one
  ...
  const handleSubmit = async (e) => {
    ...
    if (cvFile) {
        updatedCvUrl = `/${cvFile.name}`;
    }

    const res = await fetch('/api/profile', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, profileImage: previewImage, cvUrl: updatedCvUrl }),
    });
    if (res.ok) {
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="flex flex-col items-center">
                    {previewImage && previewImage.startsWith('data:') ? (
                        ...
                        <Image src={previewImage || '[https://placehold.co/150x150](https://placehold.co/150x150)'} alt="Profile Preview" width={150} height={150} className="rounded-full object-cover w-36 h-36 border-4 border-gray-700"/>
                    )}
